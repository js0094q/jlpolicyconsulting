...
export const mdxComponents: MDXComponents = {
  ...
  table: ({ children }) => (
    <div className="mt-6 overflow-x-auto">
      <table className="w-full min-w-[640px] border-collapse border-y border-[var(--color-border)] text-sm">
        {children}
      </table>
    </div>
  ),
  ...
  td: ({ children }) => <td className="border-b border-[var(--color-border)] px-3 py-2 text-[var(--color-muted)]">{children}</td>,
  a: ({ href, children }) => {
    if (!href) {
